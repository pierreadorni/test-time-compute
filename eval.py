...
def extract_answer(output_string, task_type="yes_no"):
    ...
    if position_yes_and_a == -1 and position_no_and_b == -1:
        # print(f"No answer found in the output string: {output_string}, {task_type}.")
        return -1
...
def run_eval(
    output_path: Path = Path("outputs/default_output.txt"), prompt_type: str = "vqa", decoding: str = "greedy" , n_beams: int = 1
):
    ...
    ranked_print(f"Prompt type: {prompt_type}")
    ...
    dataset = NaturalBench(
        SUFFIX_FOR_COT_VQA if prompt_type == "cot" else SUFFIX_FOR_VQA
    )
    ...
    with tqdm(
        ...
        for input_ids, image_tensor, image_sizes, indices in dataloader:
            cont = model.module.generate(
                ...
                temperature=0.8 if decoding != "greedy" else 1.0,
                num_beams=n_beams,
            ...
            text_outputs = [
                text.encode("unicode_escape").decode("utf-8") for text in text_outputs
            ]
            ...
            pbar.update(bs * accelerator.num_processes)
